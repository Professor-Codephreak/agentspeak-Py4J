...
flp_ask_ws_user(Query,Answer) :-
	...
	atom_concat('metaplanner-call ',TmpQueryAtom,QueryAtom),
	view([queryAtom,QueryAtom]),
	atomic_list_concat(['unilang-client -q -r FCMS -c "',QueryAtom,'"'],'',Command),
	shell_command_to_string(Command,Result),
	view([result,Result]),
	%% flp_ask_user(Query,Answer),
	getMetaplannerMessage(Answer).

%% getMetaplannerMessage(Answer) :-
%% 	repeat,
%% 	findall(Message,metaplannerMessage(Message),Messages),
%% 	length(Messages,L),
%% 	(   (	L > 0) ->
%% 	    (	view([messages,Messages]),Messages = [Answer|_],retractall(metaplannerMessage(_)),!) ;
%% 	    (	sleep(1),
%% 		fail)).
...
getMetaplannerMessage(Answer) :-
	...
	length(Messages,L),
	(   (	L > 0) ->
	    (	view([metaplannerMessages,Messages]),(Messages = [Answer|_] -> true ; true),retract(metaplannerMessage(Answer)),!) ;
	    (	sleep(1),
		fail)).

metaplanner_respond_to_jason(List,Result) :-
	correctLists(List,[username,UserName,message,Message]),
	assertz(metaplannerMessage(Message)).
...
metaplanner_respond_to_jason_canned(List,Answer) :-
	correctLists(List,[username,UserName,message,Message]),
	view([metaplanner_respond_to_jason_canned(Question,Answer)]),
	hasCannedReplyConcat(Message,Answer),
	view([metaplanner_respond_to_jason_canned_answer,Answer]),
	assertz(metaplannerMessage(Answer)).

cleanMetaplannerMessages :-
	retractall(metaplannerMessage(_)).